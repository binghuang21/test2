...
from envs.poolingPK.poolingPK import PoolingPK
...
class Env:
    def __init__(self, id, config):
        ...
        elif config.game_name == 'poolingPK':
            
            self.game = PoolingPK(width=256, height=256, num_sniper=config.sniper_num, draw=False,
                                    num_player=config.max_agent_num, num_rook=config.rook_num, num_counter=config.counter_num)
            self.ple = PLE(self.game, fps=30, force_fps=True, display_screen=False, reward_values=pooling_rewards,
                           resized_rows=80, resized_cols=80, num_steps=3, max_episodes=500, rng=id)
    ...
    def reset_cirruculumn(self, epoch):
        ...
        if self.config.game_name == 'traffic':
            ...
            self.game.init()
        elif self.config.game_name == 'poolingPK':
            agent_num = min(int(epoch / 4) + 2, self.config.max_agent_num) \
                if self.config.dynamic else self.config.max_agent_num
            self.game.PLAYER_NUM = agent_num
            self.game.init()
        else:
            raise Exception
