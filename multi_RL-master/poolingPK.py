...
#游戏触发事件
    ...
    def step(self, dt):
        ...
        for player in self.players:
            #player_pair = []
            for counter in self.counters:
                if (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 150) and (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) > 10):
                    self.COUNTER_BLOOD -= 10
                    self.reward[player.id] += self.rewards["positive"]
                ...
                #counter.rand_pos()
            for rook in self.rooks:
                if (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) <= 150) and (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) > 20):
                    self.ROOK_BLOOD -= 10
                    self.reward[player.id] += self.rewards["positive"]
