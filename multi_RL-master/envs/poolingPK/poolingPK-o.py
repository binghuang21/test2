...
from .agent import *
#from envs.pygamewrapper import PyGameWrapper
#from envs.poolingPK.agent import *
from utils import *
...
class PoolingPK(PyGameWrapper):
    ...
    def __init__(self, draw=False, width=100, height=100, num_sniper=0, num_player=1, num_rook=1, num_counter=0, full_blood=100):
        #key_mapping作用是什么
        ...
        #self.ROOK_ANGLE = agent_angle    #定义角度
    ...
    def step(self, dt):
        ...
        for rook in self.rooks:
            for counter in self.counters:
                ...
                if (count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1]) <= 70) and (count_distance_fast(rook.pos[0], rook.pos[1], counter.pos[0], counter.pos[1]) > 20) and (abs(counter.pos[1]-rook.pos[1]) <= 2 or abs(counter.pos[0]-rook.pos[0]) <= 2):
            ...
            for player in self.players:
                if count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1]) <= 70 and (count_distance_fast(rook.pos[0], rook.pos[1], player.pos[0], player.pos[1]) > 20) and (abs(player.pos[1]-rook.pos[1]) <= 2 or abs(player.pos[0]-rook.pos[0]) <= 2):
        ...
        for player in self.players:
            #player_pair = []
            for counter in self.counters:
                if (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 70) and (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) > 10):
                    self.COUNTER_BLOOD -= 10
                    self.reward[player.id] += self.rewards["positive"]
                elif count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) > 70 and (count_distance_fast(player.pos[0], player.pos[1], counter.pos[0], counter.pos[1]) <= 90):
                    self.reward[player.id] += self.rewards["positive"]
                ...
                #counter.rand_pos()
            for rook in self.rooks:
                if (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) <= 70) and (count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) > 20):
                    self.ROOK_BLOOD -= 10
                    self.reward[player.id] += self.rewards["positive"]
                elif count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) > 70 and count_distance_fast(player.pos[0], player.pos[1], rook.pos[0], rook.pos[1]) <= 90:
                    self.reward[player.id] += self.rewards["positive"]
...
if __name__ == "__main__":
    ...
    game = PoolingPK(width=100, height=100, num_sniper=0, num_player=1, num_rook=1, num_counter=0, full_blood=100, draw=True)
